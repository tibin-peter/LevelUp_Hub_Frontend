...
export default function MentorMessages() {
  ...
  // 1. Fetch Inboxes (Same API as Student)
  useEffect(() => {
    const fetchInboxes = async () => {
      try {
        ...
        const errMsg =
          err.response?.data?.message || "Failed to load student inboxes";
        setError(errMsg);
      }
    };
    fetchInboxes();
      ...
      try {
        ...
        ws.onclose = (e) => {
          if (e.code === 1006 && isMounted)
            setError("Connection lost. Please refresh the dashboard.");
        };
      } catch (err) {
        if (isMounted) setError("Unable to sync student history");
      } finally {
  ...
  const sendMessage = () => {
    ...
    if (socketRef.current.readyState === WebSocket.OPEN) {
      ...
      toast.error("Connecting to student...");
    }
  };
      ...
      <aside className="w-20 sm:w-80 md:w-96 border-r border-gray-100 flex flex-col bg-gray-50/50">
        <div className="p-4 sm:p-8 pb-4">
          <div className="hidden sm:flex items-center justify-between mb-6">
            <div>
              <h2 className="text-2xl font-black text-[#262626] tracking-tight">Student Hub</h2>
              <p className="text-[10px] font-black text-[#FF9500] uppercase tracking-[0.2em] mt-1">Mentor Console</p>
            </div>
            ...
            return (
              ...
              >
                ...
                <div className="relative shrink-0">
                  ...
                  <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-4 sm:h-4 bg-[#FF9500] border-2 border-white rounded-full shadow-sm"></div>
                </div>
      ...
      <main className="flex-1 flex flex-col bg-white/40">
        {activeConv ? (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black text-[#FF9500] uppercase tracking-[0.15em]">Direct Consultation</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 p-4 sm:p-10 overflow-y-auto space-y-6 sm:space-y-10 custom-scrollbar relative">
              ...
              {loading ? (
                <div className="flex flex-col items-center justify-center h-full space-y-5">
                  ...
                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.25em] animate-pulse">Establishing Secure Uplink</p>
                </div>
              ) : (
                <div className="relative z-10 space-y-8">
                  <div className="flex justify-center mb-10">
                    <div className="bg-[#262626] text-white px-6 py-2.5 rounded-full text-[8px] font-black uppercase tracking-[0.3em] shadow-2xl shadow-black/10 flex items-center gap-3 ring-1 ring-white/10">
                      <div className="w-1 h-1 bg-[#FF9500] rounded-full animate-pulse" /> Student Support Protocol Active
                    </div>
                  </div>
                      ...
                      <div key={i} className={`flex ${isMe ? "justify-end" : "justify-start"} animate-in fade-in slide-in-from-bottom-4 duration-700`}>
                        <div className={`max-w-[85%] sm:max-w-[65%] ${isMe ? "items-end" : "items-start"} flex flex-col gap-2`}>
                          <div className={`px-5 py-3.5 sm:px-7 sm:py-5 rounded-[24px] sm:rounded-[32px] text-xs sm:text-[15px] font-bold shadow-sm leading-relaxed tracking-tight ${isMe
                            ? "bg-gradient-to-br from-[#262626] to-[#404040] text-white rounded-tr-none shadow-xl shadow-black/20"
              ...
              <div className="max-w-5xl mx-auto flex items-center gap-3 sm:gap-5">
                <div className="flex-1 relative group">
                  <input
                    ...
                    placeholder={`Reply to ${(activeConv.other_user_name || activeConv.OtherUserName)}...`}
                ...
                <button
                  ...
                  className="w-14 h-14 sm:w-20 sm:h-20 bg-[#FF9500] text-white rounded-[24px] sm:rounded-[32px] flex items-center justify-center shadow-[0_20px_40px_rgba(255,149,0,0.15)] hover:bg-[#262626] hover:text-[#FF9500] hover:scale-105 active:scale-95 transition-all duration-500 disabled:opacity-20 disabled:hover:scale-100"
                >
                  <Send size={24} />
                </button>
              </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-10 sm:p-20 text-center relative overflow-hidden">
            ...
            <h3 className="text-2xl sm:text-3xl font-black text-[#262626] mb-4 tracking-tight">Student Workspace</h3>
            <p className="text-gray-400 font-bold text-xs sm:text-sm max-w-sm leading-relaxed opacity-80">
              Select a student protocol from the left to provide expert guidance and accelerate their learning journey.
            </p>
          </div>
        )}
      </main>
